 ...
 */
export function hasGameEnded(row1: string[], row2: string[], row3: string[]) {
  return checkDiagonals(row1, row2, row3) || checkRows(row1, row2, row3) || checkColumns(row1, row2, row3) || "";
}

function checkDiagonals(row1: string[], row2: string[], row3: string[]) {
  return (row1[0] === row2[1] && row2[1] === row3[2]) || (row1[2] === row2[1] && row2[1] === row3[0]) ? row2[1] : null;
}

function checkRows(row1: string[], row2: string[], row3: string[]) {
  for (let i = 0; i < 3; i++) {
...
function checkColumns(row1: string[], row2: string[], row3: string[]) {
...
function convertRowToBoard(row1: string[], row2: string[], row3: string[]) {
  let board: string[][] = [
    ["", "", ""],
    ["", "", ""],
    ["", "", ""]
 ...
 */
export function computeComputerMove(board: string[][], player: string) {
...
class TreeNode {
  ...
  children: TreeNode[];
  value: number;
  constructor(board: string[][], move: string | null, value: number = 0) {
    this.board = board;
    this.move = move;
    this.children = [];
    this.value = value;
...
router.get("/minmaxmove", async (req: Request, res: Response) => {
  if (!req.userId) return res.status(401).send("Unauthorized");
  if (!req.query.gameid) return res.status(401).send("Query board - ngid"); //nie podano ID gry
  ...
  let row: string[];
  let symbolgracza: string = "";

  // wyznaczamy jaki symbol ma kolejny ruch
  if (queryboard.user1 == queryboard.nextMove) symbolgracza = "X";
  else if (queryboard.user2 == queryboard.nextMove) symbolgracza = "O";
...
*/
  ...
  if (ktowygral === "O") kolko = true;
  else if (ktowygral === "X") krzyzyk = true;
